...
export default function PrintPage() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const response = await fetch("/api/projects?category=Print", {
          cache: "no-store", // Disable caching
        })

        if (!response.ok) {
          throw new Error("Failed to fetch print projects")
        }

        const data = await response.json()
        console.log("Fetched print projects:", data)
    ...
    <main className="min-h-screen bg-black text-white">
      <div className="container mx-auto px-4 py-20">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }}>
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Print Design</h1>
          <p className="text-zinc-400 max-w-2xl mb-12">
            A collection of print design projects including editorial, packaging, branding, and marketing materials.
          </p>
        </motion.div>

        <div className="flex overflow-x-auto pb-4 mb-8">
          {["All", "Editorial", "Branding", "Packaging", "Stationery", "Marketing", "Poster"].map((filter) => (
            <button
              key={filter}
        ...
        {isLoading ? (
          ...
          >
            {filteredProjects.map((project, index) => {
              ...
              const imageUrl = mainImage?.image || `/placeholder.svg?height=600&width=600&text=Print+${index + 1}`
          ...
          <div className="text-center py-20">
            <h3 className="text-2xl font-bold mb-4">No Print Projects Found</h3>
            <p className="text-zinc-400 mb-8">
              There are currently no print design projects available. Check back later or explore other categories.
            </p>
            <Link
              href="/projects"
